...
from .exceptions import (
    ...
    ShippingTransportNotDepartedException,
)
...
class ShippingItemSerializer(serializers.ModelSerializer):
    ...
    current_distribution_center_code = serializers.CharField(
        read_only=True, help_text="change only by transport api"
    )
    ...
    def get_shipping_batches_history(self, obj):
        return (
            obj.shipping_batches.select_related("shipping_transport")
            .all()
...
class ShippingBatchAddSerializer(serializers.Serializer):
    ...
    def save(self):
        shipping_dto = ShippingDTO(
            batch=self.context["batch"], transport=self.context["transport"]
        )
...
class ShippingItemAddSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if shippingitem.status not in [
            ShippingItemStatus.CREATED,
    ...
    def save(self):
        shipping_dto = ShippingDTO(
            batch=self.context["batch"], item=self.context["shippingitem"]
        )
...
class ShippingTransportStartSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if transport.completed:
            raise ShippingTransportCompletedException()
        if not (
            transport.distribution_center_code_source
