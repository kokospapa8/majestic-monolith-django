...
from tests.schemas.auth_schemas import token_schema
...
class TestAuthPhonenumberCheck:
    ...
    def test_phonenumber_check_user_not_exist(self, client, phonenumbers):
        ...
        response = client.post(url, {"phonenumber": phonenumber})
...
class TestAuthJWTToken:
    ...
    def test_token_refresh(self, client, user_1):
        ...
        response = client.post(url, {"refresh": str(token)})
    ...
    def test_token_refresh_invalid_token(self, client):
        url = resolve_url("token_refresh")
        response = client.post(url, {"refresh": "sadfdsfa"})
...
class TestAuthSigin:
    ...
    def test_signin_too_much_request(
        self,
        mock_bypass,
        mock_cache_count,
        mock_attempt_verification,
        client,
        phonenumbers,
    ):
    ...
    def test_signin_token_confirm_phonenumber_before_request(
        self, mock_bypass, client, phonenumbers
    ):
        url = resolve_url("signin_token_confirm")
        response = client.post(
            url,
    ...
    def test_signin_token_confirm_success(
        self, mock_bypass, mock_verify_token, client, phonenumbers
    ):
        url = resolve_url("signin_token_confirm")
        response = client.post(
            url,
    ...
    @mock.patch("auth.serializers.bypass_token_request", return_value=True)
    def test_signin_token_confirm_invalid_token(
        self, mock_bypass, mock_verify_token, client, phonenumbers
    ):
        url = resolve_url("signin_token_confirm")
        response = client.post(
    ...
    @mock.patch("auth.serializers.bypass_token_request", return_value=True)
    @mock.patch.object(SigninTokenConfirmSerializer, "verify_token", return_value=True)
    def test_signin_token_confirm_invalid_user(
        self, mock_verify_token, bypass_token_request, client, phonenumbers
    ):
        url = resolve_url("signin_token_confirm")
        response = client.post(
            url,
