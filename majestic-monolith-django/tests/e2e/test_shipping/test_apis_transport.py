...
class TestTransportScenario:
    ...
    def test_normal_transport(self, staff_user,
                              new_transport,
                              new_batch,
                              existing_shipping_item):
        client = CustomClient()
        ...
        # 3. add existing item to batch
        url = resolve_url('batch_shippingitem_add',
                          alias=batch_alias)
        post_param = {
        ...
        url = resolve_url('batch_shippingitems',
                          alias=batch_alias)
        response = client.get(url)
        assert len(response.json()['results']) == 1

        # 4. add batch to transport
        url = resolve_url('transport_batches_add',
                          uuid=transport_uuid)
        post_param = {
            "alias": batch_alias,
        }
        ...
        url = resolve_url('transport_batches',
                          uuid=transport_uuid)
        response = client.get(url)
        assert len(response.json()['results']) == 1
        ...
        url = resolve_url('transport_batches_start',
                          uuid=transport_uuid)
        response = client.post(url)
        ...
        # 6. end transport
        url = resolve_url('transport_batches_complete',
                          uuid=transport_uuid)
        response = client.post(url)
        assert response.status_code == status.HTTP_200_OK
        ...
        assert item.current_distribution_center_code == \
            new_transport['distribution_center_code_destination']
