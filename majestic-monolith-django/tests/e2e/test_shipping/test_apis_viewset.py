...
from tests.schemas.shipping_schemas import (
    shippingbatch_schema,
    shippingitem_schema,
    shippingtransport_schema,
)
...
class TestShippingItemViewSet:
    ...
    def test_shippingitem_create(self, staff_user, new_shipping_item):
        ...
        url = resolve_url(
            "shippingitem-detail", tracking_number=new_item_tracking_number
        )
        response = client.get(url)
...
class TestShippingBatchViewSet:
    ...
    def test_shippingbatch_create(self, staff_user, new_batch):
        ...
        url = resolve_url("batch-detail", alias=new_alias)
        response = client.get(url)
...
class TestShippingTransportViewSet:
    ...
    def test_shippingtransport_create(self, staff_user, new_transport):
        ...
        assert (
            response.json()["distribution_center_source"]["center_code"]
            == new_transport["distribution_center_code_source"]
        )
        assert (
            response.json()["distribution_center_destination"]["center_code"]
        ...
        url = resolve_url("transport-detail", uuid=new_uuid)
        response = client.get(url)
