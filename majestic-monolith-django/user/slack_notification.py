...
class SlackNotificationUser(BaseSlackNotification):
    ...
    def signup(self, user, fullname):
        ...
        context["data"] = (
            f"uuid: {user.uuid}, fullname:{fullname}, "
            f"phonenumber:{user.phonenumber_national_str()}"
        )
        self.send_slack("slack/ops_lastmile.slack", context, settings.SLACK_OPS_CHANNEL)
    ...
    def deacivated(self, user):
        ...
        self.send_slack("slack/ops_log.slack", context, settings.SLACK_OPS_CHANNEL)
