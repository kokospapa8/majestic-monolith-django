...
class ShippingItem(models.Model):
    ...
    status = models.CharField(
        max_length=24,
        choices=ShippingItemStatus.choices,
        blank=True,
        default=ShippingItemStatus.CREATED,
        db_index=True,
    )

    shipping_batches = models.ManyToManyField(
        "shipping.ShippingBatch",
        blank=True,
        help_text="can assign multiple shipping batches",
    )
    current_distribution_center_code = models.CharField(
        blank=True,
        null=True,
        max_length=16,
        ...
        if self.status == ShippingItemStatus.CREATED:
            if self.timestamp_completed is not None:
                raise ValidationError(
                    "Shipping status CREATED but has timestamp_completed"
                )
...
class ShippingBatch(models.Model):
    ...
    shipping_transport = models.ForeignKey(
        "shipping.ShippingTransport",
...
class ShippingTransport(models.Model):
    ...
    distribution_center_code_source = models.CharField(
        blank=True,
        null=True,
        max_length=16,
        help_text="No FK since it's in different domain",
    )
    distribution_center_code_destination = models.CharField(
        blank=True,
        null=True,
        max_length=16,
        help_text="No FK since it's in different domain",
    ...
    driver_uuid = models.UUIDField(
        blank=True, null=True, help_text="user_id, No FK since it's in different domain"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if (
            self.distribution_center_code_source
            == self.distribution_center_code_destination
        ):
            logger.error(
                f"ShippingTransport: source and destination code cannot be same."
                f"{self.distribution_center_code_source} -> "
                f"{self.distribution_center_code_destination}"
            )
            raise IntegrityError(
                "ShippingTransport: source and destination code cannot be same."
            )
        super().save(*args, **kwargs)

    def __str__(self):
        return (
            f"[ShippingTransport]{self.uuid} "
            f"{self.distribution_center_code_source} -> "
            f"{self.distribution_center_code_destination}"
        )
