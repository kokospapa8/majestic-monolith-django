...
class SignupTokenRequestSerializer(serializers.Serializer):
    ...
    def validate_phonenumber(self, attrs):
        ...
        if is_banned_phonenumber(attrs):
            raise serializers.ValidationError(
                _("This number is permanently banned.")
            )
        return attrs
...
class SignupTokenConfirmSerializer(PhonenumberCheckSerializer):
    ...
    def validate_phonenumber(self, attrs):
        ...
        if not PhonenumberCheck.objects.filter(phonenumber=attrs, verified=True).exists():
            raise serializers.ValidationError(
...
class SigninTokenConfirmSerializer(LoginSerializer):
    ...
    def validate(self, attrs):
        ...
        PhonenumberVerificationLog.objects.create(phonenumber=phonenumber,
                                                  type=PhonenumberVerificationLog.VerificationType.SIGNIN, success=is_verified)
